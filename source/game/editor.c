...
void editor_scene_init() {
  ...
  cube_model = ex_iqm_load_model(editor_scene, "data/dude.iqm", 0);
  cube_model->is_lit = 1;
  cube_model->cast_shadow = 0;
  cube_model->transform_fulls[0].scale = 0.5f;
  ex_scene_add_model(editor_scene, cube_model);

  ex_model_set_anim(cube_model, "Walk");
